...
Перед добавлением сверься с [catalogs-scope.md](../svt-architecture/catalogs-scope.md): справочник должен быть **в скоупе**; домен, видимость меню ЕО и тип работ — из реестра. Этот навык покрывает только **RO** (грид/карточка/sync). Редактор и загрузчик — отдельные этапы (roadmap 0.5–0.8).
