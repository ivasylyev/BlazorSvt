...
- Разграничение доступа — на прикладном уровне (сейчас: Windows Auth + любая роль `dbo.Role` = read;
  доменные роли записи — roadmap.md раздел 9); на
...
3. Массовый тираж read-only (MVP 0.2 / раздел 7) и контур записи (MVP 0.5+) **не** считаются
...
4. В бюджет программы **не** закладывается единый коэффициент ускорения ИИ до выполнения
   калибровки 0.4а (D-EST); точечные примеры бизнеса по отчётам (§4.5) не подменяют калибровку.
