...
полностью демонтирован, см. roadmap.md Gate D / backlog 2.0.1) — состояние во время dual-run (легаси-UI и
...
[backlog.md](backlog.md) 0.2.1 (не путать с §6.9 ниже — условное архитектурное одобрение).
...
методике «2 модели ИИ + факт» (см. roadmap.md §2 «ИИ-агенты» и backlog 0.0.5). Ценность этой оценки —
...
### 4.6 Корпоративный паспорт ИС

По карточке информационной системы «Система ведения тарифов (СВТ)», код `85`, выгрузка архитектурного контекста от 2026-09-03. Это паспорт действующей системы, не решение команды по стеку программы.
...
| Уровень защищённости ИСПДн | 4 |
| Санкционный риск | D |
| Домен карточки | SC (расшифровки в выгрузке нет) |
| Поставка | Вендорское решение с собственными разработками |
| СУБД | MS SQL Server |
| Обновление | Ручное |
| Размещение | Централизованная, типовая |
...
- Минимизировать связанность модулей — единая сборка `.csproj`, граница проверяется механически (dependency-тест в CI, см. roadmap.md §2 / backlog 0.2.2.6), а не только код-ревью
...
- После SoT flip мастером становятся только данные, порождённые в СВТ. Атрибуты из АСВ НСИ и ЦБД остаются у внешней мастер-системы (состав — [catalogs-scope.md](catalogs-scope.md), раздел «Внешние мастера»)
...
- Жёсткое удаление — только для архивных/просроченных записей с согласованным для каждого справочника лагом (например, 2 года для ставок); перенос логики хранения и очистки из легаси SQL Agent в C# — по домену, вместе с CQRS конвейер записи (детали — roadmap.md §2 строка sync, backlog 0.1.7, 0.2.2.5, 0.7.2.76 / 0.8.2.8)
...
- Разграничение доступа — на прикладном уровне (сейчас: Windows Auth + любая роль `dbo.Role` = read;
  ...
  уровне БД разграничения нет — Integrated Security, общая учётка управляемой службы (gMSA) IIS пул приложений IIS (app pool) (унаследовано
...
2. Жёлтые зоны условий 3–7 ревью ведутся как **отложенная митигация** с проверяемыми критериями
   ...
   `roadmap.md` §2б (ID D-PERF … D-DUAL). Отдельные файлы под каждый артефакт не ведутся —
...
в `roadmap.md` §2б. Закрытие риска = артефакт (приложение) или факт исполнения, не
...
Детали плана и Gate — в [roadmap.md](roadmap.md); чеклист — [backlog.md](backlog.md).

1. **Справочники только для чтения (read-only)** — 63 в скоупе (38 вне скоупа), 7 групп-доменов (Ставки, Маршруты, Продукты, Склады и ограничения, Матрица каналов, Интеграция с ЦБД, Вспомогательные); состав — catalogs-scope.md; watermark-sync легаси → грид
...
└── architecture/
    ...
    # отложенная митигация (threat model, SoD, SLI/SLO, …) — в roadmap.md §2б,
