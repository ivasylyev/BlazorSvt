...
- На переходный период SoT = легаси-таблицы (write-through из нового UI); после SoT flip — новый контур
...
- Бесшовный переход (Strangler Fig); SoT = легаси-таблицы до cutover; exit dual-run волнами → SoT flip в конце
...
1. **Справочники read-only** — ~60 используемых (~40 вне скоупа); watermark-sync легаси → грид
...
4. **Exit dual-run волнами** → **SoT flip** после OPE и решения бизнеса (MVP 2.0)
