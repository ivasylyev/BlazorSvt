...
BEGIN
    ...
    -- Основная таблица: изменившиеся PrimitiveEntityItemId и есть бизнес-ключи рейтов.
    IF @Source = N'dbo.PrimitiveEntityData_2012'
    BEGIN
        INSERT INTO #AffectedKeys (EntityKey)
        SELECT d.PrimitiveEntityItemId
        FROM dbo.PrimitiveEntityData_2012 d WITH (NOLOCK)
    ...
    IF @Source = N'dbo.PrimitiveEntityData_1013'
    BEGIN
        INSERT INTO #AffectedKeys (EntityKey)
        SELECT DISTINCT s.TransportRateId
        FROM dbo.PrimitiveEntityData_1013 r WITH (NOLOCK)
    ...
    IF @Source = N'dbo.PrimitiveEntityData_1015'
    BEGIN
        INSERT INTO #AffectedKeys (EntityKey)
        SELECT DISTINCT s.TransportRateId
        FROM dbo.PrimitiveEntityData_1015 r WITH (NOLOCK)
