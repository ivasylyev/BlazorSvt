...
через RabbitMQ, частично напрямую, частично через БД — визуальное подтверждение «распределённого
...
32 ГБ под IIS/Blazor Server; 4 ядра под MS SQL Server. Предварительные факты нагрузочного тестирования
...
на что этот сценарий уже сейчас влияет: текущий выбор session affinity для Blazor Server (см. §6.1)
...
- Предварительно подтверждено фактом на MVP 0.1 (текущий объём данных, не целевой 10 млн —
  ...
  тяжёлой карточке — 3% CPU, ~1,5 МБ памяти/сессию на app-сервере; массовое изменение 200К
  записей — sync за 30 сек, <1% нагрузки на app-сервер, ~15% CPU на SQL Server (4 ядра)
...
- RTO/RPO: унаследованы от инфраструктуры легаси (общая ВМ и общая БД) — отдельный DR-контур
...
- Инфраструктурная доступность (98,5%) наследуется от легаси, но новый прикладной контур вносит
  собственные, не унаследованные сценарии недоступности (обрыв Blazor-circuit при recycle app pool,
  отказ sync-воркера, стопор доменного write-toggle, ошибка конфигурации URL Rewrite) — явный
