...
![Легаси-архитектура As-Is](../../../docs/architecture/diagrams/01-as-is-legacy.png)
...
> Глоссарий узких терминов (SoT, write-through, dual-run, watermark, CQRS и др.) — в [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §1.2а.
...
![Целевая архитектура текущей программы](../../../docs/architecture/diagrams/02-to-be-current-program.png)

Та же ВМ и тот же Windows Server 2016/IIS, что и в легаси (см. [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §1.3 — «дочернее
...
полностью демонтирован, см. [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) Gate D / backlog 2.0.1) — состояние во время dual-run (легаси-UI и
новый UI одновременно в одном IIS-сайте, переключаемые через URL Rewrite) отдельно не
диаграммировано и описано текстом в [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §1.3.
...
[03. Бэклог трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=583080778) 0.2.1 (не путать с §6.9 ниже — условное архитектурное одобрение).
...
> стороне БД. Согласно backlog 1.2.1, при выносе оркестрации из SQL Server Agent сама
...
![Возможное будущее состояние — контейнеризация](../../../docs/architecture/diagrams/03-future-containers-out-of-scope.png)

Контейнеризация (Docker/Kubernetes) **не входит** в текущую программу трансформации
(MVP 0.0–2.0) и нигде не фигурирует в бэклоге. Показана здесь как возможный
...
- На переходный период SoT = легаси-таблицы (сквозная запись (write-through) из нового UI); Смена источника истины (SoT flip) — инкрементальная, по доменам справочников, после чего write-through продолжается до финального демонтажа легаси (детали — в [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766), §2а)
...
> сопоставимых операциях; дополняют предварительные факты нагрузочного тестирования (backlog
> 0.2.1), но получены отдельно от них, силами бизнеса.
...
(см. backlog 0.2.1.2) в однопользовательском режиме. Это не заменяет полный протокол
нагрузочного тестирования при пиковой конкурентности читателей (backlog 0.2.1.3/0.2.1.10), но
...
методике «2 модели ИИ + факт» (см. [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §2 «ИИ-агенты» и backlog 0.0.5). Ценность этой оценки —
...
Эта оценка не заменяет формальную калибровку по категориям работ (backlog 0.0.5) — примеры
...
принят и вынесен на проверку в ранний нагрузочный PoC (backlog 0.2.1).
...
- Минимизировать связанность модулей — единая сборка `.csproj`, граница проверяется механически (dependency-тест в CI, см. [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §2 / backlog 0.2.2.6), а не только код-ревью
...
- Жёсткое удаление — только для архивных/просроченных записей с согласованным для каждого справочника лагом (например, 2 года для ставок); перенос логики хранения и очистки из легаси SQL Agent в C# — по домену, вместе с CQRS конвейер записи (детали — [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §2 строка sync, backlog 0.1.7, 0.2.2.5, 0.7.2.76 / 0.8.2.8)
...
- Предварительно подтверждено фактом на MVP 0.1 (текущий объём данных, не целевой 10 млн —
  полная проверка на целевом объёме в backlog 0.2.1): 100 конкурентных Blazor-сессий на самой
...
- Осознанное, точечное исключение из принципа «логика не в БД»: SP-трансформация
  ...
  SQL Server Agent только оркестрация (backlog 1.2.1, MVP 1.2)
...
- Разграничение доступа — на прикладном уровне (сейчас: Windows Auth + любая роль `dbo.Role` = read;
  доменные роли записи — backlog 0.4.1); на
...
- Хостинг: новое приложение — дочернее приложение в том же IIS-сайте, что и легаси (`PathBase`),
  ...
  (детали — [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §1.3)
- целевое время восстановления / допустимая потеря данных (RTO/RPO): унаследованы от инфраструктуры легаси (общая ВМ и общая БД) — отдельный DR-контур
  ...
  благодаря общей БД (детали, текущие значения и план улучшения — [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §2, строка «RTO/RPO»)
- Инфраструктурная доступность (98,5%) наследуется от легаси, но новый прикладной контур вносит
  ...
  перечень и митигации см. [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §2.1
- Подключение алертов sync/ёмкости к существующему мониторингу и процессу эскалации инцидентов
  техподдержки — в работе (заявка направлена, см. backlog 0.1.7.8); формальное письменное
  подтверждение ИБ, что read-only фаза (MVP 0.1–0.3) не увеличивает риск относительно легаси, —
  запрошено (см. backlog 0.2.2.7)
- Сканирование зависимостей на известные уязвимости (SCA) — через GitLab SAST/Sonar в CI,
  настройка запланирована, ещё не выполнена (см. backlog 0.1.4.6)

### 6.9 Условное архитектурное одобрение

Документация претендует на **условное** согласование направления трансформации и программы
пилотирования, а не на безусловное закрытие всех условий повторного architecture review.

**Одобряется при условиях:**
1. Направление (модульный монолит, SQL-only FTS, поэтапная замена (Strangler Fig), CQRS,
   Gate A/B/C/D) признаётся целевым для текущей программы.
2. Жёлтые зоны условий 3–7 ревью ведутся как **отложенная митигация** с проверяемыми критериями
   закрытия — канонический реестр и полный текст приложений (модель угроз (threat model),
   разделение обязанностей (SoD), SLI/SLO, миграции, метрики пилотов, протокол нагрузки):
   [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §2б (ID D-PERF … D-DUAL). Отдельные файлы под каждый артефакт не ведутся —
   всё в одном разделе дорожной карты, чтобы занятому ревьюеру не прыгать по папке.
3. Массовый тираж read-only (MVP 0.2 / backlog 0.2) и контур записи (MVP 0.5+) **не** считаются
   разрешёнными этим же решением, пока не закрыты соответствующие строки реестра §2б
   (как минимум D-PERF, D-ARCH, D-SEC-RO для тиража; D-DATA-MIG и артефакты ИБ — для записи).
4. В бюджет программы **не** закладывается единый коэффициент ускорения ИИ до выполнения
   калибровки 0.0.5 (D-EST); точечные примеры бизнеса по отчётам (§4.5) не подменяют калибровку.

**Зачем так:** остаточные риски (residual risk) не «под ковром», а связаны с Gate и текстом
в [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §2б. Закрытие риска = артефакт (приложение) или факт исполнения, не
переформулировка текста. Снимок повторного ревью — `docs/architecture/review-current-state.html`.

## 7. Дорожная карта реализации (текущий проект)

Детали плана и Gate — в [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766); чеклист — [03. Бэклог трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=583080778).

1. **Справочники только для чтения (read-only)** — 63 в скоупе (38 вне скоупа), 7 групп-доменов (Ставки, Маршруты, Продукты, Склады и ограничения, Матрица каналов, Интеграция с ЦБД, Вспомогательные); состав — [04. Список справочников для трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573822390); watermark-sync легаси → грид
...
└── architecture/
    ...
    └── review-current-state.html — снимок повторного architecture review
    # отложенная митигация (threat model, SoD, SLI/SLO, …) — в [02. План трансформации СВТ](https://confluence.sibur.local/pages/viewpage.action?pageId=573815766) §2б,
